    ...
    <article className="midi article-card">
      <div className="article-card-inner d-flex flex-column flex-md-row">
        ...
        <div className="article-content-wrapper flex-grow-1">
          ...
          <p className="article-preview text-muted mb-3">{previewText}</p>

          {/* Теги */}
          {displayTags.length > 0 && (
            <div className="article-tags mb-3 d-flex gap-3">
              {displayTags.map((tag) => (
          ...
          <div className="d-flex justify-content-between align-items-center gap-3">
            <Link to={`/article/${data.id}`} className="btn-3 d-flex w-100">
              {t("Читать статью")}
            </Link>
